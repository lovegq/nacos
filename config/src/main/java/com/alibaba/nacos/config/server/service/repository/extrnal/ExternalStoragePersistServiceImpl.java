...
public class ExternalStoragePersistServiceImpl implements PersistService {
	...
	public void addConfigInfo4Tag(ConfigInfo configInfo, String tag, String srcIp, String srcUser, Timestamp time,
		...
		try {
			jt.update(
					...
					configInfo.getDataId(), configInfo.getGroup(), tenantTmp, tagTmp, appNameTmp,
	...
	public void updateConfigInfo(final ConfigInfo configInfo, final String srcIp, final String srcUser,
		...
		boolean result = tjt.execute(status -> {
			try {
				...
				/*
				 * If the appName passed by the user is not empty, use the persistent user's
				 * appName, otherwise use db; when emptying appName, you need to pass an empty
				 * string
				 */
	...
	public Page<ConfigInfo> findAllConfigInfo(final int pageNo, final int pageSize, final String tenant) {
		...
		try {
			return helper.fetchPageLimit(sqlCountRows, sqlFetchRows,
					new Object[] { generateLikeArgument(tenantTmp), pageSize, (pageNo - 1) * pageSize }, pageNo,
	...
	public List<ConfigInfo> findConfigInfoByBatch(final List<String> dataIds, final String group, final String tenant,
		...
		// Batch query limit
		// The number of in is controlled within 100, the shorter the length of the SQL
		...
		for (int i = 0; i < dataIds.size(); i += subQueryLimit) {
			...
			List<ConfigInfo> r = this.jt.query(sqlStart + subQuerySql.toString() + sqlEnd, params.toArray(),
					CONFIG_INFO_ROW_MAPPER);
	...
	public Page<ConfigInfo> findConfigInfoLike(final int pageNo, final int pageSize, final ConfigKey[] configKeys,
		...
		// Whitelist, please leave the synchronization condition empty, there is no
		// configuration that meets the conditions
	...
	public ConfigInfoAggr findSingleConfigInfoAggr(String dataId, String group, String tenant, String datumId) {
		...
		try {
			...
			// EmptyResultDataAccessException, indicating that the data does not exist,
			// returns null
			return null;
		} catch (CannotGetJdbcConnectionException e) {
			LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
	...
	public Page<ConfigInfoAggr> findConfigInfoAggrByPage(String dataId, String group, String tenant, final int pageNo,
		...
		String sqlFetchRows = "select data_id,group_id,tenant_id,datum_id,app_name,content from config_info_aggr where data_id=? and "
	...
	public long addConfigInfoAtomic(final long configId, final String srcIp, final String srcUser,
		...
		final String sql = "INSERT INTO config_info(data_id,group_id,tenant_id,app_name,content,md5,src_ip,src_user,gmt_create,"
	...
	public Page<ConfigHistoryInfo> findConfigHistory(String dataId, String group, String tenant, int pageNo,
		...
		String sqlFetchRows = "SELECT nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified FROM his_config_info "
	...
	public List<ConfigInfoWrapper> listGroupKeyMd5ByPage(int pageNo, int pageSize) {
		...
		try {
			Page<ConfigInfoWrapper> page = helper.fetchPageLimit(sqlCountRows, sqlFetchRows,
					new Object[] { pageSize, (pageNo - 1) * pageSize }, pageNo, pageSize,
					CONFIG_INFO_WRAPPER_ROW_MAPPER);
	...
	public int tenantInfoCountByTenantId(String tenantId) {
		...
		Integer result = this.jt.queryForObject(SQL_TENANT_INFO_COUNT_BY_TENANT_ID, new String[] { tenantId },
				Integer.class);
		if (result == null) {
			return 0;
		}
